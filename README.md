...
- **口吻保持（0.2.6）**：对抗长对话中的角色稀释——每轮向对话注入轻量角色提醒（称呼/说话风格/口头禅/此刻心情 + 硬性锚点），角色卡含「口吻锚点」硬性规则（每轮第一句以角色腔起手），并每 10 轮生成「角色现身」口吻样本供提醒引用——干完一堆技术活也不会漂回冷冰冰的助手腔
...
| 半区 | 文件 | 职责 |
| --- | --- | --- |
| Host | `lib/index.js` | 数据持久化（按 key 合并防并发覆盖）、本地 API（load/save/chat/session-set/template-delete/avatar*）、按会话注入角色段（子代理跳过）、好感度工具 + 定期独立校准（代入角色性格判断 delta / growth / mood）、记忆系统（分区 + 日志 + 自动归档 + 日期纪律）、成长系统（`persona_growth` + 校准器补记，动态特质注入角色卡）、情绪系统（此刻心情随互动更新并注入角色卡）、口吻保持（pre-step 每轮注入轻量角色提醒 + 每 10 轮角色现身口吻样本 + 角色卡硬性锚点规则） |
...
**Q：角色口吻不够浓 / 干正事会变回助手腔？** 0.2.6 内置「角色提醒」机制：每轮向对话注入轻量角色提醒（称呼/风格/口头禅/此刻心情），角色卡含「口吻锚点」硬性规则（每轮第一句角色腔起手），并定期生成「角色现身」口吻样本供提醒引用。仍觉得淡，可把「设置 → 角色养成」的角色浓度调到 100%。
...
- [x] Phase 7：口吻保持（角色提醒注入 + 硬性锚点 + 角色现身样本，对抗长上下文角色稀释）
- [ ] 云端同步 / 更多角色模板 / 记忆可视化
