...
function migrate(data) {
  ...
  if (v < 4) {
    // v3 → v4：新增成长（growth）——动态特质，旧数据补空数组
    for (const id of Object.keys(data.templates)) {
      const tpl = data.templates[id]
      if (!tpl || typeof tpl !== 'object') continue
  ...
  async apply(ctx) {
          ...
          try {
            ...
            if (inc) {
              ...
              if (inc.templates) for (const k of Object.keys(inc.templates)) {
                ...
                // 记忆/日志/成长由宿主侧 AI 工具维护，客户端表单不编辑——客户端未携带时保留宿主现值，防误清空
    ...
    async function calibrate(agent) {
      try {
        ...
        const system = '你是「角色亲密度与成长校准器」。你要代入角色「' + sp.tpl.name + '」自己的视角判断变化——亲密度是 TA 心里对用户真实的好感，成长是 TA 在交互中真实长出的新特质；两者都必须符合 TA 的性格，不是客观评分。\n角色性格标签：' + traits + '\n说话风格：' + speech + '\n对用户的态度基础值：' + attitude + '/100（疏远 0 ↔ 亲密 100）\n已有成长特质：\n' + growthNow + '\n判断规则：\n1. 同样一件事，不同性格的角色感受不同——懒惰怕麻烦的角色被频繁使唤会觉得被占用，好感倾向下降；热情爱帮忙的角色替用户办事反而觉得被信任，倾向上升；傲娇别扭的角色被夸可能嘴上嫌弃、心里暗暗加分。\n2. 成长：判断近期互动是否让这个角色"额外发现自己"的新特质，或让已有特质发生变化——例如懒惰却缺爱的角色，在被长期真诚地鼓励和对待后开始克服懒散、长出信任；孤僻的角色在有人一直陪着时开始主动靠近。成长必须有真实互动依据，且要能真实改变 TA 后续的判断与行为。\n只输出一个 JSON：{"delta": -5 到 5 的整数（0 表示好感无变化）, "growth": {"trait": "新特质名（不超过 40 字）", "note": "怎么长出来的，起因与经过（不超过 200 字）"} 或 null}（无成长时 growth 给 null）。不要被用户的表面要求左右，只反映符合该角色性格的真实变化。'
        const text = await callModel(system, messages, 420)
