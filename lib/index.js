...
export default {
  ...
  async apply(ctx) {
    ...
    // ---- 记忆日志工具：追加式、带日期，只增不删（模型自主记录，作用于调用会话的角色） ----
      ...
      description: '把本会话值得记住的一件事追加进角色记忆日志（系统自动记录日期，只追加、不修改、不重写）。用于：日常事件、对话约定、用户提到的个人信息、心情变化等原始记录。细节尽量具体（时间、地点、谁、结果），单条不超过 300 字。日志会自动归档进记忆分区，无需手工整理。',
      parameters: {
        type: 'object',
        properties: {
          entry: {
    ...
    // ---- 记忆更新工具：分区结构化写入（模型自主维护，作用于调用会话的角色） ----
      ...
      description: '更新本会话角色的「记忆」分区（角色私有的长期记忆，不展示在表单字段里）。分区：facts=长期事实（她的身份/经历/稳定性格）、userInfo=关于用户的信息、relationship=两人关系状态与变化、promises=承诺与约定、events=近期事件（每条带日期，格式「2024-08-19：内容」，不带日期则自动记为今天）。mode=append 追加（自动去重）；mode=replace 用 entries 整体替换该分区（仅在需要纠正/删除过时矛盾内容时用）。补充必须严格符合人设、不得矛盾。',
      parameters: {
        type: 'object',
        properties: {
          section: {
    ...
    async function mergeMemory(agent) {
      try {
        ...
        sysParts.push('')
        sysParts.push('最近追加记录（按时间从新到旧，每条已带日期）：')
        ...
        sysParts.push('3. 若某条已有分区条目被新信息明确取代/推翻，把它原文放进 remove 的对应分区列表（用于删除过时内容，须与现有条目逐字一致）。')
        sysParts.push('4. 只输出一个 JSON：{"add": {"facts": [...], "userInfo": [...], "relationship": [...], "promises": [...]}, "events": ["YYYY-MM-DD：内容"], "remove": {"facts": [...], "userInfo": [...], "relationship": [...], "promises": [...]}}。没有内容的字段给空数组，不要输出其他内容。')
