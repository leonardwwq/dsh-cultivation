...
`;
      ...
      api("load").then((r) => {
        ...
        if (r.data.templates && d.templates) {
          for (const id of Object.keys(d.templates)) {
            ...
            // 记忆/日志/成长由宿主侧 AI 工具维护，宿主为准（兼容 v2 字符串与 v3 分区结构）
            if (ht.memory) d.templates[id].memory = ht.memory;
            if (Array.isArray(ht.memoryLog)) d.templates[id].memoryLog = ht.memoryLog;
            if (Array.isArray(ht.growth)) d.templates[id].growth = ht.growth;
            if (ht.mood && typeof ht.mood === "object") d.templates[id].mood = ht.mood;
          }
        }
    ...
    function openEditor(id) {
      setStore({ editingTemplateId: id, open: true, isNew: false, draft: null, saveState: "idle" });
    }
    function openNewRole() {
      setStore({ open: true, isNew: true, editingTemplateId: null, draft: { id: "draft", name: "新角色", intimacy: 0, fields: emptyFields(), memory: { facts: [], userInfo: [], relationship: [], promises: [], events: [] }, memoryLog: [], growth: [], mood: null }, saveState: "idle" });
    }
    function saveNow() {
      const s = getStore();
      if (s.isNew && s.draft) {
    ...
    function exportRole(id) {
      ...
      const payload = { app: "dsh-cultivation", version: 5, name: tpl.name, fields: tpl.fields, memory: tpl.memory && typeof tpl.memory === "object" ? tpl.memory : (typeof tpl.memory === "string" ? tpl.memory : ""), memoryLog: Array.isArray(tpl.memoryLog) ? tpl.memoryLog : [], growth: Array.isArray(tpl.growth) ? tpl.growth : [], mood: tpl.mood && typeof tpl.mood === "object" ? tpl.mood : null };
      try {
      ...
      reader.onload = () => {
        try {
          ...
          s.data.templates[id] = {
            ...
            growth: Array.isArray(p.growth) ? p.growth : [],
            mood: p.mood && typeof p.mood === "object" ? p.mood : null
          };
          bump();
          showToast("角色已导入");
    ...
    function renderCard(data, templateId) {
      ...
      const growth = Array.isArray(tpl.growth) ? tpl.growth.filter((g) => g && g.trait).map((g) => "- " + g.trait + (g.since ? "（" + g.since + "起）" : "") + (g.note ? "：" + g.note : "")) : [];
      if (growth.length) parts.push("【角色成长（交互中真实长出的新特质，必须真实影响你的判断与行为，不是摆设）】\n" + growth.join("\n"));
      const mood = tpl.mood && typeof tpl.mood === "object" && tpl.mood.text ? String(tpl.mood.text) : "";
      if (mood) parts.push("【此刻心情】" + mood + (tpl.mood.since ? "（" + tpl.mood.since + "）" : ""));
    ...
    async function apply(ctx) {
      ...
      if (!data) {
        data = { version: 5, defaultPersonaId: null, defaultEnabled: true, settings: { concentration: 80, showIntimacyBadge: true }, templates: { "tpl-init": { id: "tpl-init", name: "新角色", intimacy: 0, fields: emptyFields(), memory: { facts: [], userInfo: [], relationship: [], promises: [], events: [] }, memoryLog: [], growth: [], mood: null } }, sessions: {} };
      }
