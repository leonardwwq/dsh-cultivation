// 角色工坊 (@persona-studio/dsh) —— Client 半区
// 会话级角色模型：
...
`;
    ...
    const CSS_TAG = "@persona-studio/dsh/styles";
      ...
      api("load").then((r) => {
        ...
        if (r.data.templates && d.templates) {
          for (const id of Object.keys(d.templates)) {
            ...
            if (typeof ht.memory === "string") d.templates[id].memory = ht.memory;
          }
        }
    ...
    // 从 AI 回复里去掉 ```json 代码块（JSON 只用于回填表单，不展示）
    function stripJsonBlock(text) {
      return String(text || "").replace(/```(?:json)?\s*[\s\S]*?```/g, "").replace(/\n{3,}/g, "\n\n").trim();
    }
    ...
    function saveNow() {
      ...
      api("save", { data: d }).then(() => { setStore({ saveState: "saved" }); showToast("已保存"); }).catch(() => setStore({ saveState: "saved" }));
    }
    ...
    function exportRole(id) {
      ...
      const payload = { app: "persona-studio", version: 1, name: tpl.name, fields: tpl.fields, memory: typeof tpl.memory === "string" ? tpl.memory : "" };
      try {
    ...
    function renderCard(data, templateId) {
      ...
      const memory = typeof tpl.memory === "string" && tpl.memory ? tpl.memory : "";
      if (memory) parts.push("【角色记忆（角色私有，AI 自主补充）】\n" + memory);
      return parts.join("\n\n");
    }
      ...
      const send = async () => {
        ...
        try {
          ...
          if (r && r.patch && typeof r.patch === "object") {
            ...
            if (n > 0) showToast("已根据对话更新 " + n + " 个字段");
          }
        } catch (e) {
